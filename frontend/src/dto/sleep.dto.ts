class SleepDto {
  ...
  constructor(data: Event) {
    ...
    this.link = data?.link;
    this.extraLink = data.menu;
    this.dateTime = data.hours?.display;
    this.latitude = data?.geocodes?.main?.latitude.toString();
    this.longitude = data?.geocodes?.main?.longitude.toString();
    this.picture = data?.photos?.[0]?.prefix + data?.photos?.[0]?.suffix;
    this.priceRangeMin = data?.price;
    this.priceRangeMax = data?.price;
  }
}

export default SleepDto;
